...
class PostRes(nn.Module):
    def __init__(self, n_in, n_out, stride = 1,num_group=32):
        super(PostRes, self).__init__()
        self.bn1 = nn.BatchNorm3d(n_in)
        #self.relu = nn.Softplus(beta=1, threshold=20)
        self.relu=nn.ReLU(inplace=False) 
        self.conv1 = nn.Conv3d(n_in, n_out, kernel_size = (3,5,3), stride = stride, padding = (1,2,1))
        self.bn2 = nn.BatchNorm3d(n_out)
        self.conv2 = nn.Conv3d(n_out, n_out, kernel_size = (3,3,5), padding = (1,1,2),groups=num_group)
...
class Loss(nn.Module):
    def __init__(self, num_hard = 0):
        ...
        #self.classify_loss = nn.KLDivLoss()
