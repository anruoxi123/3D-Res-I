...
fold = 6
annotations_filename = '/7T_DISK/mhq/3in1-D/evaluationScript/annotations/annotations.csv'# path for ground truth annotations for the fold
annotations_excluded_filename ='/7T_DISK/mhq/3in1-D/evaluationScript/annotations/annotations_excluded.csv' # path for excluded annotations for the fold
seriesuids_filename = '/7T_DISK/mhq/3in1-D/evaluationScript/annotations/seriesuids_'+str(fold)+'.csv'# path for seriesuid for the fold
results_path = '/7T_DISK/mhq/3in1-D/detector/results/res18/retrft96'+str(fold)+'/val' 
# val' ft96'+'/val'#
sideinfopath = '/7T_DISK/mhq/3in1-D/luna16/preprocess/subset'+str(fold)+'/'#subset'+str(fold)+'/'  +str(fold)
datapath = '/7T_DISK/mhq/3in1-D/luna16/data/subset'+str(fold)+'/'#subset'+str(fold)+'/'

maxeps = 200 #03 #150 #100#100
eps = range(180, maxeps+1, 1)#6,7,1)#5,151,5)#5,151,5)#76,77,1)#40,41,1)#76,77,1)#1,101,1)#17,18,1)#38,39,1)#1, maxeps+1, 1) #maxeps+1, 1)
...
def convertcsv(bboxfname, bboxpath, detp):
    ...
    # print len(pbb), pbb[0]
    #print '333+',bboxfname, pbbold.shape, pbb.shape, pbbold.shape
    ...
    #print '666+',len(rowlist), len(rowlist[0])
    
    return rowlist#bboxfname[:-8], pos[:K, 2], pos[:K, 1], pos[:K, 0], 1/(1+np.exp(-pbb[:K,0]))
def getfrocvalue(results_filename):
    return noduleCADEvaluation(annotations_filename,annotations_excluded_filename,seriesuids_filename,results_filename,'./')#vis=False)   , vis=isvis
p = Pool(nprocess)
def getcsv(detp, eps):
    for ep in eps:
